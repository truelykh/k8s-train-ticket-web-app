...
public class RoutePlanServiceImpl implements RoutePlanService {
    ...
    public Response searchMinStopStations(RoutePlanInfo info, HttpHeaders headers) {
        ...
        ArrayList<Route> routeList = responseList(re, Route.class);
        if (routeList.isEmpty()) {
            LOGGER.warn("[searchMinStopStations][No usable routes returned][From: {}, To: {}]", fromStationId, toStationId);
            return new Response<>(1, "Success.", new ArrayList<>());
        }
    ...
    private <T> ArrayList<T> responseList(ResponseEntity<Response<Object>> response, Class<T> elementType) {
        ArrayList<T> values = new ArrayList<>();
        if (response == null || !response.getStatusCode().is2xxSuccessful() || response.getBody() == null) {
            return values;
        }
        Object payload = response.getBody().getData();
        if (!(payload instanceof List<?>)) {
            LOGGER.warn("[responseList][Invalid upstream list payload][Expected: {}, Actual: {}]",
                    elementType.getSimpleName(), payload == null ? "null" : payload.getClass().getName());
            return values;
        }
        for (Object value : (List<?>) payload) {
            if (elementType.isInstance(value)) {
                values.add(elementType.cast(value));
            }
        }
        return values;
    }
