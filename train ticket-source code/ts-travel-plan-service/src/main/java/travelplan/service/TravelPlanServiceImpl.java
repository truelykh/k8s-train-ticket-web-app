...
public class TravelPlanServiceImpl implements TravelPlanService {
    ...
    private int getRestTicketNumber(String travelDate, String trainNumber, String startStationName, String endStationName, int seatType, int totalNum, List<String> stations, HttpHeaders headers) {
        ...
        ResponseEntity<Response<Object>> re = restTemplate.exchange(
        ...
        if (re == null || !re.getStatusCode().is2xxSuccessful() || re.getBody() == null) {
            LOGGER.warn("[getRestTicketNumber][Seat response unavailable][Train: {}, Seat type: {}]", trainNumber, seatType);
            return 0;
        }

        Object payload = re.getBody().getData();
        if (payload instanceof Number) {
            return Math.max(0, ((Number) payload).intValue());
        }
        if (payload instanceof String) {
            try {
                return Math.max(0, Integer.parseInt((String) payload));
            } catch (NumberFormatException ignored) {
                // Treat invalid upstream data as no available tickets rather than failing the request.
            }
        }

        LOGGER.warn("[getRestTicketNumber][Invalid seat response payload][Train: {}, Seat type: {}, Payload type: {}]",
                trainNumber, seatType, payload == null ? "null" : payload.getClass().getName());
        return 0;
    }
