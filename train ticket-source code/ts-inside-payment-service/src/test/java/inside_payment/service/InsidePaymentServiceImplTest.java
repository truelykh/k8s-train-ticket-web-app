...
public class InsidePaymentServiceImplTest {
    ...
    public void testCreateAccount2() {
        ...
        List<Money> list = new ArrayList<>();
        list.add(new Money());
        Mockito.when(addMoneyRepository.findByUserId(Mockito.nullable(String.class))).thenReturn(list);
        Response result = insidePaymentServiceImpl.createAccount(info, headers);
    ...
    public void testInitPayment1() {
        ...
        Mockito.when(paymentRepository.findById(Mockito.nullable(String.class))).thenReturn(java.util.Optional.empty());
        Mockito.when(paymentRepository.save(Mockito.any(Payment.class))).thenReturn(null);
    ...
    public void testInitPayment2() {
        Payment payment = new Payment();
        Mockito.when(paymentRepository.findById(Mockito.nullable(String.class))).thenReturn(java.util.Optional.ofNullable(payment));
        Mockito.when(paymentRepository.save(Mockito.any(Payment.class))).thenReturn(null);
        insidePaymentServiceImpl.initPayment(payment, headers);
