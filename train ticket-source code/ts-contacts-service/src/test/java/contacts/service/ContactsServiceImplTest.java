...
public class ContactsServiceImplTest {
    ...
    public void testCreateContacts1() {
        Contacts contacts = new Contacts();
        Mockito.when(contactsRepository.findByAccountIdAndDocumentTypeAndDocumentType(Mockito.any(), Mockito.any(), Mockito.anyInt())).thenReturn(contacts);
    ...
    public void testCreateContacts2() {
        Contacts contacts = new Contacts();
        Mockito.when(contactsRepository.findByAccountIdAndDocumentTypeAndDocumentType(Mockito.any(), Mockito.any(), Mockito.anyInt())).thenReturn(null);
    ...
    public void testCreate2() {
        ...
        Mockito.when(contactsRepository.save(Mockito.any(Contacts.class))).thenReturn(null);
        Response result = contactsServiceImpl.create(addContacts, headers);
        Assert.assertEquals(new Response<>(1, "Create contacts success", null), result);
    }

    @Test
    public void testDelete1() {
        UUID contactsId = UUID.randomUUID();
        Mockito.doNothing().doThrow(new RuntimeException()).when(contactsRepository).deleteById(Mockito.anyString());
        Mockito.when(contactsRepository.findById(Mockito.anyString())).thenReturn(Optional.empty());
